...
namespace dukat
{
    FullscreenEffect2::FullscreenEffect2(Game2* game) : game(game), anim(nullptr), color({0.f, 0.f, 0.f, 0.f})
    {
    }
    ...
    void FullscreenEffect2::create_fade_layer(void)
    {
        ...
        layer->set_composite_binder([&](ShaderProgram* p) {
            p->set("u_diffuse", color.r, color.g, color.b, color.a);
        });
    }
    ...
    void FullscreenEffect2::fade_in(float duration, std::function<void(void)> callback)
    {
        create_fade_layer();
        color.a = 1.0f;
        auto value_anim = std::make_unique<ValueAnimation<float>>(&color.a, duration, 0.0f);
    ...
    void FullscreenEffect2::fade_out(float duration, std::function<void(void)> callback)
    {
        create_fade_layer();
        color.a = 0.0f;
        auto value_anim = std::make_unique<ValueAnimation<float>>(&color.a, duration, 1.0f);
        value_anim->set_callback([&, callback](void) {
            if (callback != nullptr)
                callback();
            anim = nullptr;
