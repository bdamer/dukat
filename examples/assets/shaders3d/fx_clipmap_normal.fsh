#version 150
///
// Fragment shader to generate clipmap normal.
///
in vec2 v_tex_coord;
...
void main()
{
    ...
    vec2 texcoord0 = (vec2(max(0, uv.x - 1), uv.y) + u_texture_offset) * u_one_over_size;
    float z1 = texture(u_tex0, vec3(texcoord0 + 0.5 * u_one_over_size, u_level)).r;  
    vec2 texcoord1 = (vec2(min(u_size, uv.x + 1), uv.y) + u_texture_offset) * u_one_over_size;
    float z2 = texture(u_tex0, vec3(texcoord1 + 0.5 * u_one_over_size, u_level)).r;
    float zx = z2 - z1;
    
    // compute a local tangent vector along the Y axis
    texcoord0 = (vec2(uv.x, max(0, uv.y - 1)) + u_texture_offset) * u_one_over_size;
    z1 = texture(u_tex0, vec3(texcoord0 + 0.5 * u_one_over_size, u_level)).r; 
    texcoord1 = (vec2(uv.x, min(u_size, uv.y + 1)) + u_texture_offset) * u_one_over_size;
    z2 = texture(u_tex0, vec3(texcoord1 + 0.5 * u_one_over_size, u_level)).r;
    float zy = z2 - z1;
